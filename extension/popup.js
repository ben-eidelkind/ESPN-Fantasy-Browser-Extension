// popup.js — matches popup.html IDs; robust Detect + existing behaviors
...
function updateActionButtons() {
  ...
  togglePreviewButton.textContent = previewExpanded ? "Show more" : "Show more"; // text toggled in renderPreview
...
// Show more detailed failure messages (adds error.message when present)
...
// --------- FIXED: robust leagueId detection that matches popup.html IDs ---------
async function detectLeagueIdFromTab() {
  ...
  // 2) try to parse directly from tab.url (fast path)
  try {
    const m = String(tab.url || "").match(/[?&#]leagueId=(\d+)/);
    if (m) return m[1];
  } catch (_) {}

  // 3) ask TOP FRAME content script
  ...
  // 4) fallback — ask any frame
  const anyResp = await new Promise((resolve) => {
